...
- **主更新 100% 在 GitHub Actions**（`update.yml`，每日 08:00 UTC，数据变化才 commit；9 步管道：GH Archive 241h 下载→WatchEvent 统计→验星→筛选→DeepSeek 翻译→写 JSON→deploy→push gh-pages）
- **新鲜度报警已迁 GHA**：`freshness-check.yml` 每日 09:00（北京）检查线上数据（≤1 项 / updated 超 2 天 → 失败 + PushPlus 微信通知）
- 依赖仅 requests；DeepSeek key 在 GitHub Secrets；本地开发 key 在 `C:\Users\63966\.secrets\hermes-identity\github-cn-nav.md`
...
1. 凭证统一存 `.secrets\hermes-identity\github-cn-nav.md`，不归 git
...
# 从 .secrets\hermes-identity\github-cn-nav.md 读 GITHUB_TOKEN 和 DEEPSEEK_API_KEY 设环境变量
